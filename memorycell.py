...
import numpy as np
from flipflop_01 import MAGNET_CURRENT
import pxi_modules
...
#AWG_OPTIMAL_AMPLITUDES = [0.45, 0.45, 0.7]      # for 10 MHz
AWG_OPTIMAL_AMPLITUDES = [0.4, 0.4, 0.6]      # for 10 MHz
...
INPUT_DELAY = int(1e9)//(2*BIT_RATE) # delay input signal by 1/2 of symbol period (relative to read clock)
...
if SAVE_TRANSIENT:
    with open(f'csvs/memory_cell/memory_cell_transient_{int(BIT_RATE/1e6)}MHz_{timestamp}.npy', 'wb') as f:
        np.savez(
            f,
            timestamp=timestamp,
            bitrate=BIT_RATE,
...
try:
    ...
    for cfg in range(N_CONFIGS):
        ...
        for n,channel in enumerate(DAQ_CHANNELS):
            ...
            threshold = 0.5*np.max(daq_data[n])
...
print("BER = ", ber)
with open(f'csvs/memory_cell/memory_cell_ber_{int(BIT_RATE/1e6)}MHz_{timestamp}.npy', 'wb') as f:
    np.savez(
        f,
        timestamp=timestamp,
        bitrate=BIT_RATE,
...
if SAVE == 1:
    ...
    mat = sio.savemat(f"csvs/memory_cell/memory_cell_ber_{timestamp}.mat", save_data)
...
if not(DEBUG_BER_PLOT):
    ...
    plt.savefig(f'csvs/memory_cell/memory_cell_ber_{int(BIT_RATE/1e6)}MHz_{timestamp}.png', bbox_inches='tight')
    plt.show()
